	...
	public static void main(String[] args) {
	    ...
	    loadTimings.lineTimings.entrySet().stream().forEach(entry -> {
	    	System.out.printf(Locale.ENGLISH, " %3d,  %3d,     %7.3f,     %7.3f,     %7.3f,     %7.3f%n",
	    			entry.getKey(),
	    			entry.getValue().size(),
	    			entry.getValue().stream().mapToDouble(a 
	    					-> a.timeNextPoint).average().getAsDouble()/(double)entry.getKey(),
	    			entry.getValue().stream().mapToDouble(a 
	    					-> a.timeGetX).average().getAsDouble()/(double)entry.getKey(),
	    			entry.getValue().stream().mapToDouble(a 
	    					-> a.timeGetY).average().getAsDouble()/(double)entry.getKey(),
	    			entry.getValue().stream().mapToDouble(a 
	    					-> a.timeTotal).average().getAsDouble()/(double)entry.getKey()
			);
	    });
	    System.out.println();
	    ...
	    double timeTotalAllLines = loadTimings.lineTimings.values().stream().flatMap(v 
	    		-> v.stream()).mapToDouble(v -> v.timeTotal).sum();
	    double timeTotalCount = loadTimings.lineTimings.values().stream().flatMap(v 
	    		-> v.stream()).mapToDouble(v -> v.timeTotal).count();
	    System.out.printf(Locale.ENGLISH, "Avg nextLine call: %4.3fms%n",
	    		(loadTimings.timeNextLine/(double)timeTotalCount));
	    System.out.printf(Locale.ENGLISH, "Avg load time per line: %4.3fms%n",
	    		(timeTotalAllLines/(double)timeTotalCount));
	    ...
	    System.out.println();
	    System.out.println(" NoP   NoL     slope[ms]  intercept in[ms]     total[ms]    summed[ms]");
	    isValidTimes.entrySet().stream().forEach(entry -> {
	    	System.out.printf(Locale.ENGLISH, " %3d,  %3d,     %7.6f,     %7.6f,     %7.6f,     %7.6f%n",
	    			entry.getKey(),
	    			entry.getValue().size(),
	    			entry.getValue().stream().mapToDouble(a
	    					-> a[0]).average().getAsDouble()/1000000.0,
	    			entry.getValue().stream().mapToDouble(a
	    					-> a[1]).average().getAsDouble()/1000000.0,
	    			entry.getValue().stream().mapToDouble(a
	    					-> a[2]).average().getAsDouble()/1000000.0,
	    			entry.getValue().stream().mapToDouble(a
	    					-> a[0]).sum()/1000000.0
			);
	    });
